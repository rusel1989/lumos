# Change Log

All notable changes to this project will be documented in this file.
See [Conventional Commits](https://conventionalcommits.org) for commit guidelines.

### 2.4.121 - 2021-04-27

**Note:** Version bump only for package @oriflame/lumos





### 2.4.120 - 2021-04-25

**Note:** Version bump only for package @oriflame/lumos





### 2.4.119 - 2021-04-24

**Note:** Version bump only for package @oriflame/lumos





### 2.4.118 - 2021-04-23

**Note:** Version bump only for package @oriflame/lumos





### 2.4.117 - 2021-04-23

#### 📦 Dependencies

- update oriflame dependencies ([448ed48](https://github.com/Oriflame/lumos/commit/448ed4855399fd29b0dcf57b50a742dd89467220))

**Note:** Version bump only for package @oriflame/lumos





### 2.4.116 - 2021-04-22

**Note:** Version bump only for package @oriflame/lumos





### 2.4.115 - 2021-04-21

#### 📦 Dependencies

- update dependency chalk to ^4.1.1 ([74203bb](https://github.com/Oriflame/lumos/commit/74203bbb2eaebd38717143865a48ff059ee6e016))

**Note:** Version bump only for package @oriflame/lumos





### 2.4.114 - 2021-04-21

#### 🐞 Fixes

- Fix ESLint issues. (#526) ([e7783e8](https://github.com/Oriflame/lumos/commit/e7783e8b76c24b45a9162f824c45397006aed79e)), closes  _#526_

**Note:** Version bump only for package @oriflame/lumos





### 2.4.113 - 2021-04-20

**Note:** Version bump only for package @oriflame/lumos





### 2.4.112 - 2021-04-20

**Note:** Version bump only for package @oriflame/lumos





### 2.4.111 - 2021-04-17

**Note:** Version bump only for package @oriflame/lumos





### 2.4.110 - 2021-04-15

**Note:** Version bump only for package @oriflame/lumos





### 2.4.109 - 2021-04-15

**Note:** Version bump only for package @oriflame/lumos





### 2.4.108 - 2021-04-13

#### 📦 Dependencies

- bump @actions/core from 1.2.6 to 1.2.7 (#503) ([8377c91](https://github.com/Oriflame/lumos/commit/8377c913a8054e2d0f02e6f720e62deb14145139)), closes  _#503_

**Note:** Version bump only for package @oriflame/lumos





### 2.4.107 - 2021-04-13

**Note:** Version bump only for package @oriflame/lumos





### 2.4.106 - 2021-04-12

**Note:** Version bump only for package @oriflame/lumos





### 2.4.105 - 2021-04-12

**Note:** Version bump only for package @oriflame/lumos





### 2.4.104 - 2021-04-11

**Note:** Version bump only for package @oriflame/lumos





### 2.4.103 - 2021-04-10

**Note:** Version bump only for package @oriflame/lumos





### 2.4.102 - 2021-04-09

**Note:** Version bump only for package @oriflame/lumos





### 2.4.101 - 2021-04-09

**Note:** Version bump only for package @oriflame/lumos





### 2.4.100 - 2021-04-08

#### 🐞 Fixes

- Add eslint tests and fix webpack issue. (#477) ([a6fc8e5](https://github.com/Oriflame/lumos/commit/a6fc8e52fee830101877bb42649ce677afb3b6c0)), closes  _#477_

**Note:** Version bump only for package @oriflame/lumos





### 2.4.99 - 2021-04-08

**Note:** Version bump only for package @oriflame/lumos





### 2.4.98 - 2021-04-08

**Note:** Version bump only for package @oriflame/lumos





### 2.4.97 - 2021-04-06

**Note:** Version bump only for package @oriflame/lumos





### 2.4.96 - 2021-04-06

**Note:** Version bump only for package @oriflame/lumos





### 2.4.95 - 2021-04-05

**Note:** Version bump only for package @oriflame/lumos





### 2.4.94 - 2021-04-03

**Note:** Version bump only for package @oriflame/lumos





### 2.4.93 - 2021-04-02

**Note:** Version bump only for package @oriflame/lumos





### 2.4.92 - 2021-04-01

**Note:** Version bump only for package @oriflame/lumos





### 2.4.91 - 2021-03-31

**Note:** Version bump only for package @oriflame/lumos





### 2.4.90 - 2021-03-28

**Note:** Version bump only for package @oriflame/lumos





### 2.4.89 - 2021-03-28

**Note:** Version bump only for package @oriflame/lumos





### 2.4.88 - 2021-03-28

**Note:** Version bump only for package @oriflame/lumos





### 2.4.87 - 2021-03-26

**Note:** Version bump only for package @oriflame/lumos





### 2.4.86 - 2021-03-25

**Note:** Version bump only for package @oriflame/lumos





### 2.4.85 - 2021-03-25

#### 📦 Dependencies

- bump @oriflame/conventional-changelog from 1.0.9 to 1.1.0 (#445) ([808b6fe](https://github.com/Oriflame/lumos/commit/808b6feb6c0b9f307ddc4fae86d18d0f083f50b0)), closes  _#445_

**Note:** Version bump only for package @oriflame/lumos





### 2.4.84 - 2021-03-24

**Note:** Version bump only for package @oriflame/lumos





### 2.4.83 - 2021-03-23

**Note:** Version bump only for package @oriflame/lumos





### 2.4.82 - 2021-03-23

**Note:** Version bump only for package @oriflame/lumos





### 2.4.81 - 2021-03-22

**Note:** Version bump only for package @oriflame/lumos





### 2.4.80 - 2021-03-21

**Note:** Version bump only for package @oriflame/lumos





### 2.4.79 - 2021-03-21

**Note:** Version bump only for package @oriflame/lumos





### 2.4.78 - 2021-03-18

**Note:** Version bump only for package @oriflame/lumos





### 2.4.77 - 2021-03-17

**Note:** Version bump only for package @oriflame/lumos





### 2.4.76 - 2021-03-16

**Note:** Version bump only for package @oriflame/lumos





### 2.4.75 - 2021-03-15

**Note:** Version bump only for package @oriflame/lumos





### 2.4.74 - 2021-03-15

**Note:** Version bump only for package @oriflame/lumos





### 2.4.73 - 2021-03-09

**Note:** Version bump only for package @oriflame/lumos





### 2.4.72 - 2021-03-09

**Note:** Version bump only for package @oriflame/lumos





### 2.4.71 - 2021-03-08

**Note:** Version bump only for package @oriflame/lumos





### 2.4.70 - 2021-03-08

**Note:** Version bump only for package @oriflame/lumos





### 2.4.69 - 2021-03-06

**Note:** Version bump only for package @oriflame/lumos





### 2.4.68 - 2021-03-04

**Note:** Version bump only for package @oriflame/lumos





### 2.4.67 - 2021-03-02

#### 📦 Dependencies

- bump edit-json-file from 1.5.0 to 1.6.0 (#378) ([8ed4aa1](https://github.com/Oriflame/lumos/commit/8ed4aa1)), closes [#378](https://github.com/Oriflame/lumos/issues/378)

**Note:** Version bump only for package @oriflame/lumos





### 2.4.66 - 2021-03-01

**Note:** Version bump only for package @oriflame/lumos





### 2.4.65 - 2021-03-01

**Note:** Version bump only for package @oriflame/lumos





### 2.4.64 - 2021-03-01

**Note:** Version bump only for package @oriflame/lumos





### 2.4.63 - 2021-03-01

**Note:** Version bump only for package @oriflame/lumos





### 2.4.62 - 2021-02-26

**Note:** Version bump only for package @oriflame/lumos





### 2.4.61 - 2021-02-26

**Note:** Version bump only for package @oriflame/lumos





### 2.4.60 - 2021-02-26

**Note:** Version bump only for package @oriflame/lumos





### 2.4.59 - 2021-02-25

**Note:** Version bump only for package @oriflame/lumos





### 2.4.58 - 2021-02-25

**Note:** Version bump only for package @oriflame/lumos





### 2.4.57 - 2021-02-24

**Note:** Version bump only for package @oriflame/lumos





### 2.4.56 - 2021-02-24

**Note:** Version bump only for package @oriflame/lumos





### 2.4.55 - 2021-02-24

**Note:** Version bump only for package @oriflame/lumos





### 2.4.54 - 2021-02-24

**Note:** Version bump only for package @oriflame/lumos





### 2.4.53 - 2021-02-23

**Note:** Version bump only for package @oriflame/lumos





### 2.4.52 - 2021-02-23

**Note:** Version bump only for package @oriflame/lumos





### 2.4.51 - 2021-02-22

**Note:** Version bump only for package @oriflame/lumos





### 2.4.50 - 2021-02-22

**Note:** Version bump only for package @oriflame/lumos





### 2.4.49 - 2021-02-22

**Note:** Version bump only for package @oriflame/lumos





### 2.4.48 - 2021-02-22

**Note:** Version bump only for package @oriflame/lumos





### 2.4.47 - 2021-02-20

**Note:** Version bump only for package @oriflame/lumos





### 2.4.46 - 2021-02-19

**Note:** Version bump only for package @oriflame/lumos





### 2.4.45 - 2021-02-19

**Note:** Version bump only for package @oriflame/lumos





### 2.4.44 - 2021-02-18

**Note:** Version bump only for package @oriflame/lumos





### 2.4.43 - 2021-02-17

**Note:** Version bump only for package @oriflame/lumos





### 2.4.42 - 2021-02-16

**Note:** Version bump only for package @oriflame/lumos





### 2.4.41 - 2021-02-16

**Note:** Version bump only for package @oriflame/lumos





### 2.4.40 - 2021-02-16

**Note:** Version bump only for package @oriflame/lumos





### 2.4.39 - 2021-02-13

**Note:** Version bump only for package @oriflame/lumos





### 2.4.38 - 2021-02-13

**Note:** Version bump only for package @oriflame/lumos





### 2.4.37 - 2021-02-13

**Note:** Version bump only for package @oriflame/lumos





### 2.4.36 - 2021-02-13

**Note:** Version bump only for package @oriflame/lumos





### 2.4.35 - 2021-02-12

**Note:** Version bump only for package @oriflame/lumos





### 2.4.34 - 2021-02-12

**Note:** Version bump only for package @oriflame/lumos





### 2.4.33 - 2021-02-11

**Note:** Version bump only for package @oriflame/lumos





### 2.4.32 - 2021-02-10

**Note:** Version bump only for package @oriflame/lumos





### 2.4.31 - 2021-02-09

**Note:** Version bump only for package @oriflame/lumos





### 2.4.30 - 2021-02-08

**Note:** Version bump only for package @oriflame/lumos





### 2.4.29 - 2021-02-06

**Note:** Version bump only for package @oriflame/lumos





### 2.4.28 - 2021-02-05

#### 📦 Dependencies

- bump check-node-version from 4.0.3 to 4.1.0 (#281) ([b233f79](https://github.com/Oriflame/lumos/commit/b233f79)), closes [#281](https://github.com/Oriflame/lumos/issues/281)

**Note:** Version bump only for package @oriflame/lumos





### 2.4.27 - 2021-02-04

**Note:** Version bump only for package @oriflame/lumos





### 2.4.26 - 2021-02-04

**Note:** Version bump only for package @oriflame/lumos





### 2.4.25 - 2021-02-04

**Note:** Version bump only for package @oriflame/lumos





### 2.4.24 - 2021-02-04

**Note:** Version bump only for package @oriflame/lumos





### 2.4.23 - 2021-02-04

**Note:** Version bump only for package @oriflame/lumos





### 2.4.22 - 2021-02-04

**Note:** Version bump only for package @oriflame/lumos





### 2.4.21 - 2021-02-03

**Note:** Version bump only for package @oriflame/lumos





### 2.4.20 - 2021-02-01

**Note:** Version bump only for package @oriflame/lumos





### 2.4.19 - 2021-02-01

**Note:** Version bump only for package @oriflame/lumos





### 2.4.18 - 2021-01-31

**Note:** Version bump only for package @oriflame/lumos





### 2.4.17 - 2021-01-30

**Note:** Version bump only for package @oriflame/lumos





### 2.4.16 - 2021-01-28

**Note:** Version bump only for package @oriflame/lumos





### 2.4.15 - 2021-01-27

**Note:** Version bump only for package @oriflame/lumos





### 2.4.14 - 2021-01-26

**Note:** Version bump only for package @oriflame/lumos





### 2.4.13 - 2021-01-25

**Note:** Version bump only for package @oriflame/lumos





### 2.4.12 - 2021-01-25

**Note:** Version bump only for package @oriflame/lumos





### 2.4.11 - 2021-01-24

**Note:** Version bump only for package @oriflame/lumos





### 2.4.10 - 2021-01-24

**Note:** Version bump only for package @oriflame/lumos





### 2.4.9 - 2021-01-14

**Note:** Version bump only for package @oriflame/lumos





### 2.4.8 - 2021-01-13

**Note:** Version bump only for package @oriflame/lumos





### 2.4.7 - 2021-01-12

**Note:** Version bump only for package @oriflame/lumos





### 2.4.6 - 2021-01-12

**Note:** Version bump only for package @oriflame/lumos





### 2.4.5 - 2021-01-11

**Note:** Version bump only for package @oriflame/lumos





### 2.4.4 - 2021-01-10

**Note:** Version bump only for package @oriflame/lumos





### 2.4.3 - 2021-01-09

**Note:** Version bump only for package @oriflame/lumos





### 2.4.2 - 2021-01-09

**Note:** Version bump only for package @oriflame/lumos





### 2.4.1 - 2021-01-08

#### 📦 Dependencies

- bump @oriflame/conventional-changelog from 1.0.8 to 1.0.9 (#215) ([6839e7f](https://github.com/Oriflame/lumos/commit/6839e7f)), closes [#215](https://github.com/Oriflame/lumos/issues/215)

**Note:** Version bump only for package @oriflame/lumos





## 2.4.0 - 2021-01-05

#### 🚀 Updates

- Add webpack host setting. (#210) ([b1c1eb2](https://github.com/Oriflame/lumos/commit/b1c1eb2)), closes [#210](https://github.com/Oriflame/lumos/issues/210)

**Note:** Version bump only for package @oriflame/lumos





### 2.3.32 - 2021-01-05

**Note:** Version bump only for package @oriflame/lumos





### 2.3.31 - 2021-01-05

**Note:** Version bump only for package @oriflame/lumos





### 2.3.30 - 2021-01-04

**Note:** Version bump only for package @oriflame/lumos





### 2.3.29 - 2021-01-03

**Note:** Version bump only for package @oriflame/lumos





### 2.3.28 - 2021-01-03

#### 📦 Dependencies

- bump @oriflame/conventional-changelog from 1.0.7 to 1.0.8 (#202) ([6968a01](https://github.com/Oriflame/lumos/commit/6968a01)), closes [#202](https://github.com/Oriflame/lumos/issues/202)

**Note:** Version bump only for package @oriflame/lumos





### 2.3.27 - 2021-01-02

**Note:** Version bump only for package @oriflame/lumos





### 2.3.26 - 2021-01-01

**Note:** Version bump only for package @oriflame/lumos





### 2.3.25 - 2020-12-30

**Note:** Version bump only for package @oriflame/lumos





### 2.3.24 - 2020-12-30

**Note:** Version bump only for package @oriflame/lumos





### 2.3.23 - 2020-12-29

**Note:** Version bump only for package @oriflame/lumos





### 2.3.22 - 2020-12-28

**Note:** Version bump only for package @oriflame/lumos





### 2.3.21 - 2020-12-28

**Note:** Version bump only for package @oriflame/lumos





### 2.3.20 - 2020-12-25

**Note:** Version bump only for package @oriflame/lumos





### 2.3.19 - 2020-12-24

#### 📦 Dependencies

- bump @oriflame/conventional-changelog from 1.0.6 to 1.0.7 (#186) ([0b7d51e](https://github.com/Oriflame/lumos/commit/0b7d51e)), closes [#186](https://github.com/Oriflame/lumos/issues/186)

**Note:** Version bump only for package @oriflame/lumos





### 2.3.18 - 2020-12-24

**Note:** Version bump only for package @oriflame/lumos





### 2.3.17 - 2020-12-23

**Note:** Version bump only for package @oriflame/lumos





### 2.3.16 - 2020-12-22

**Note:** Version bump only for package @oriflame/lumos





### 2.3.15 - 2020-12-22

**Note:** Version bump only for package @oriflame/lumos





### 2.3.14 - 2020-12-21

**Note:** Version bump only for package @oriflame/lumos





### 2.3.13 - 2020-12-18

**Note:** Version bump only for package @oriflame/lumos





### 2.3.12 - 2020-12-18

**Note:** Version bump only for package @oriflame/lumos





### 2.3.11 - 2020-12-18

**Note:** Version bump only for package @oriflame/lumos





### 2.3.10 - 2020-12-15

**Note:** Version bump only for package @oriflame/lumos





### 2.3.9 - 2020-12-15

**Note:** Version bump only for package @oriflame/lumos





### 2.3.8 - 2020-12-15

**Note:** Version bump only for package @oriflame/lumos





### 2.3.7 - 2020-12-14

**Note:** Version bump only for package @oriflame/lumos





### 2.3.6 - 2020-12-11

**Note:** Version bump only for package @oriflame/lumos





### 2.3.5 - 2020-12-10

**Note:** Version bump only for package @oriflame/lumos





### 2.3.4 - 2020-12-08

**Note:** Version bump only for package @oriflame/lumos





### 2.3.3 - 2020-12-07

**Note:** Version bump only for package @oriflame/lumos





### 2.3.2 - 2020-12-06

**Note:** Version bump only for package @oriflame/lumos





### 2.3.1 - 2020-12-06

**Note:** Version bump only for package @oriflame/lumos





## 2.3.0 - 2020-12-04

#### 🚀 Updates

- **[config-eslint]** New typescript rules. (#138) ([f31518e](https://github.com/Oriflame/lumos/commit/f31518e)), closes [#138](https://github.com/Oriflame/lumos/issues/138)

**Note:** Version bump only for package @oriflame/lumos





### 2.2.2 - 2020-12-03

**Note:** Version bump only for package @oriflame/lumos





### 2.2.1 - 2020-12-03

#### 📦 Dependencies

- bump execa from 4.1.0 to 5.0.0 (#135) ([2b42d0d](https://github.com/Oriflame/lumos/commit/2b42d0d)), closes [#135](https://github.com/Oriflame/lumos/issues/135)

**Note:** Version bump only for package @oriflame/lumos





## 2.2.0 - 2020-12-03

#### 🚀 Updates

- Change arrow parent to always. ([7089e17](https://github.com/Oriflame/lumos/commit/7089e17))

**Note:** Version bump only for package @oriflame/lumos





### 2.1.16 - 2020-12-02

#### 📦 Dependencies

- bump cross-env from 7.0.2 to 7.0.3 (#131) ([25d7e19](https://github.com/Oriflame/lumos/commit/25d7e19)), closes [#131](https://github.com/Oriflame/lumos/issues/131)

**Note:** Version bump only for package @oriflame/lumos





### 2.1.15 - 2020-12-01

**Note:** Version bump only for package @oriflame/lumos





### 2.1.14 - 2020-12-01

**Note:** Version bump only for package @oriflame/lumos





### 2.1.13 - 2020-12-01

**Note:** Version bump only for package @oriflame/lumos





### 2.1.12 - 2020-11-29

**Note:** Version bump only for package @oriflame/lumos





### 2.1.11 - 2020-11-28

**Note:** Version bump only for package @oriflame/lumos





### 2.1.10 - 2020-11-27

**Note:** Version bump only for package @oriflame/lumos





### 2.1.9 - 2020-11-26

**Note:** Version bump only for package @oriflame/lumos





### 2.1.8 - 2020-11-26

**Note:** Version bump only for package @oriflame/lumos





### 2.1.7 - 2020-11-25

**Note:** Version bump only for package @oriflame/lumos





### 2.1.6 - 2020-11-24

**Note:** Version bump only for package @oriflame/lumos





### 2.1.5 - 2020-11-24

**Note:** Version bump only for package @oriflame/lumos





### 2.1.4 - 2020-11-24

**Note:** Version bump only for package @oriflame/lumos





### 2.1.3 - 2020-11-23

**Note:** Version bump only for package @oriflame/lumos





### 2.1.2 - 2020-11-22

**Note:** Version bump only for package @oriflame/lumos





### 2.1.1 - 2020-11-22

**Note:** Version bump only for package @oriflame/lumos





## 2.1.0 - 2020-11-20

#### 🚀 Updates

- **[config-typescript]** Update jsx runtime. (#97) ([a534512](https://github.com/Oriflame/lumos/commit/a534512)), closes [#97](https://github.com/Oriflame/lumos/issues/97)

**Note:** Version bump only for package @oriflame/lumos





### 2.0.31 - 2020-11-20

**Note:** Version bump only for package @oriflame/lumos





### 2.0.30 - 2020-11-19

**Note:** Version bump only for package @oriflame/lumos





### 2.0.29 - 2020-11-19

#### 🐞 Fixes

- Republish packages. ([3ac6f6b](https://github.com/Oriflame/lumos/commit/3ac6f6b))

**Note:** Version bump only for package @oriflame/lumos





### 2.0.28 - 2020-11-19

**Note:** Version bump only for package @oriflame/lumos





### 2.0.27 - 2020-11-19

**Note:** Version bump only for package @oriflame/lumos





### 2.0.26 - 2020-11-18

**Note:** Version bump only for package @oriflame/lumos





### 2.0.25 - 2020-11-18

**Note:** Version bump only for package @oriflame/lumos





### 2.0.24 - 2020-11-17

**Note:** Version bump only for package @oriflame/lumos





### 2.0.23 - 2020-11-17

**Note:** Version bump only for package @oriflame/lumos





### 2.0.22 - 2020-11-14

**Note:** Version bump only for package @oriflame/lumos





### 2.0.21 - 2020-11-12

**Note:** Version bump only for package @oriflame/lumos





### 2.0.20 - 2020-11-12

**Note:** Version bump only for package @oriflame/lumos





### 2.0.19 - 2020-11-11

**Note:** Version bump only for package @oriflame/lumos





### 2.0.18 - 2020-11-11

**Note:** Version bump only for package @oriflame/lumos





### 2.0.17 - 2020-11-11

**Note:** Version bump only for package @oriflame/lumos





### 2.0.16 - 2020-11-07

**Note:** Version bump only for package @oriflame/lumos





### 2.0.15 - 2020-11-07

**Note:** Version bump only for package @oriflame/lumos





### 2.0.14 - 2020-11-07

**Note:** Version bump only for package @oriflame/lumos





### 2.0.13 - 2020-11-06

**Note:** Version bump only for package @oriflame/lumos





### 2.0.12 - 2020-11-06

#### 📦 Dependencies

- bump @oriflame/conventional-changelog from 1.0.4 to 1.0.6 (#59) ([6627d2e](https://github.com/Oriflame/lumos/commit/6627d2e)), closes [#59](https://github.com/Oriflame/lumos/issues/59)

**Note:** Version bump only for package @oriflame/lumos





### 2.0.11 - 2020-11-05

**Note:** Version bump only for package @oriflame/lumos





### 2.0.10 - 2020-11-04

**Note:** Version bump only for package @oriflame/lumos





### 2.0.9 - 2020-11-03

**Note:** Version bump only for package @oriflame/lumos





### 2.0.8 - 2020-11-02

**Note:** Version bump only for package @oriflame/lumos





### 2.0.7 - 2020-11-02

**Note:** Version bump only for package @oriflame/lumos





### 2.0.6 - 2020-11-02

**Note:** Version bump only for package @oriflame/lumos





### 2.0.5 - 2020-11-01

**Note:** Version bump only for package @oriflame/lumos





### 2.0.4 - 2020-10-30

**Note:** Version bump only for package @oriflame/lumos





### 2.0.3 - 2020-10-28

#### 📦 Dependencies

- Update to latest. ([3006695](https://github.com/Oriflame/lumos/commit/3006695))

**Note:** Version bump only for package @oriflame/lumos





### 2.0.2 - 2020-10-28

**Note:** Version bump only for package @oriflame/lumos





### 2.0.1 - 2020-10-28

**Note:** Version bump only for package @oriflame/lumos





# 2.0.0 - 2020-10-27

#### 💥 Breaking

- Migrate to webpack 5. (#13) ([5a41615](https://github.com/Oriflame/lumos/commit/5a41615)), closes [#13](https://github.com/Oriflame/lumos/issues/13)

**Note:** Version bump only for package @oriflame/lumos





### 1.0.2 - 2020-10-17

#### 📦 Dependencies

- Update to latest. ([91729b1](https://github.com/Oriflame/lumos/commit/91729b1))

**Note:** Version bump only for package @oriflame/lumos





### 1.0.1 - 2020-10-16

#### 🛠 Internals

- Remove rajzik deps. (#11) ([346d8da](https://github.com/Oriflame/lumos/commit/346d8da)), closes [#11](https://github.com/Oriflame/lumos/issues/11)

**Note:** Version bump only for package @oriflame/lumos





### 0.1.1 - 2020-10-16

#### 🐞 Fixes

- Naming. ([3910092](https://github.com/Oriflame/lumos/commit/3910092))

**Note:** Version bump only for package @oriflame/lumos





# 0.1.0 - 2020-10-16

#### 💥 Breaking

- Next config. ([9c9588a](https://github.com/Oriflame/lumos/commit/9c9588a))
- Rewrite to oriflame package. (#2) ([42efea6](https://github.com/Oriflame/lumos/commit/42efea6)), closes [#2](https://github.com/Oriflame/lumos/issues/2)
- Update jest and remove setting. (#705) ([a994b0a](https://github.com/Oriflame/lumos/commit/a994b0a)), closes [#705](https://github.com/Oriflame/lumos/issues/705)
- Update webpack config. ([e203466](https://github.com/Oriflame/lumos/commit/e203466))

#### 🚀 Updates

- Accept args. ([b9dd423](https://github.com/Oriflame/lumos/commit/b9dd423))
- Add alias pattern setting. ([6a7f0c8](https://github.com/Oriflame/lumos/commit/6a7f0c8))
- Add devserverContentBase path. (#632) ([7a1284d](https://github.com/Oriflame/lumos/commit/7a1284d)), closes [#632](https://github.com/Oriflame/lumos/issues/632)
- Add empty option for babel. ([1b8640a](https://github.com/Oriflame/lumos/commit/1b8640a))
- Add support for alias. ([0ee7796](https://github.com/Oriflame/lumos/commit/0ee7796))
- Add support for junit test result. ([fa45aac](https://github.com/Oriflame/lumos/commit/fa45aac))
- Add testing library to eslint. ([f18bca1](https://github.com/Oriflame/lumos/commit/f18bca1))
- Add testing library to lumos. ([e0f04ec](https://github.com/Oriflame/lumos/commit/e0f04ec))
- Add unique jsonp function name. ([eb8b444](https://github.com/Oriflame/lumos/commit/eb8b444))
- added dotfiles. ([53c3e81](https://github.com/Oriflame/lumos/commit/53c3e81))
- Alias resolver for app. ([9357f94](https://github.com/Oriflame/lumos/commit/9357f94))
- Migrate to LTS node. ([6625184](https://github.com/Oriflame/lumos/commit/6625184))
- Migrate to new packages. (#87) ([4c15f2c](https://github.com/Oriflame/lumos/commit/4c15f2c)), closes [#87](https://github.com/Oriflame/lumos/issues/87)
- Move to typescript. ([294b4fb](https://github.com/Oriflame/lumos/commit/294b4fb))
- Update commitlint. ([052d624](https://github.com/Oriflame/lumos/commit/052d624))
- Update packages. ([ba09f76](https://github.com/Oriflame/lumos/commit/ba09f76))
- Updated commitlintrc. ([306a40d](https://github.com/Oriflame/lumos/commit/306a40d))

#### 🐞 Fixes

- **[webpack]** Update webpack config. ([5048a54](https://github.com/Oriflame/lumos/commit/5048a54))
- Add next.config to gitignore. ([9202678](https://github.com/Oriflame/lumos/commit/9202678))
- Add typescript option. ([94b8ecf](https://github.com/Oriflame/lumos/commit/94b8ecf))
- added stuff to gitignore. ([9b0c395](https://github.com/Oriflame/lumos/commit/9b0c395))
- Better naming. ([dddaca7](https://github.com/Oriflame/lumos/commit/dddaca7))
- Eslint issues. ([e0077fc](https://github.com/Oriflame/lumos/commit/e0077fc))
- general fixes. ([09a6b30](https://github.com/Oriflame/lumos/commit/09a6b30))
- Jest get config. ([844f5c1](https://github.com/Oriflame/lumos/commit/844f5c1))
- lint tests only when tests are enabled. ([2605f2d](https://github.com/Oriflame/lumos/commit/2605f2d))
- lintstaged. ([8a38e12](https://github.com/Oriflame/lumos/commit/8a38e12))
- lumos cross-env added. ([128fef8](https://github.com/Oriflame/lumos/commit/128fef8))
- lumos setup. ([9721811](https://github.com/Oriflame/lumos/commit/9721811))
- Migration to new octokit. ([e64c932](https://github.com/Oriflame/lumos/commit/e64c932))
- Next config. ([9c97a0b](https://github.com/Oriflame/lumos/commit/9c97a0b))
- Next js full support. ([7f38c41](https://github.com/Oriflame/lumos/commit/7f38c41))
- only for release. ([5ef2c58](https://github.com/Oriflame/lumos/commit/5ef2c58))
- preffer local. ([22ab97a](https://github.com/Oriflame/lumos/commit/22ab97a))
- redeploy. ([406909d](https://github.com/Oriflame/lumos/commit/406909d))
- Remove next options from setting. ([3dbdf4f](https://github.com/Oriflame/lumos/commit/3dbdf4f))
- removed lintstagedrc. ([c8afeb8](https://github.com/Oriflame/lumos/commit/c8afeb8))
- scaffold fix. ([0252b67](https://github.com/Oriflame/lumos/commit/0252b67))
- Support for js. ([6d71e84](https://github.com/Oriflame/lumos/commit/6d71e84))
- test ([4c2b8ed](https://github.com/Oriflame/lumos/commit/4c2b8ed))
- Typescript config. ([3d59972](https://github.com/Oriflame/lumos/commit/3d59972))
- typo. ([646d391](https://github.com/Oriflame/lumos/commit/646d391))
- typo. ([07dba35](https://github.com/Oriflame/lumos/commit/07dba35))
- Update babel driver. ([fe1b611](https://github.com/Oriflame/lumos/commit/fe1b611))
- Update ignore configs. ([fc01217](https://github.com/Oriflame/lumos/commit/fc01217))
- updated scaffold. ([c672c99](https://github.com/Oriflame/lumos/commit/c672c99))
- Webpack config. ([b6cbc35](https://github.com/Oriflame/lumos/commit/b6cbc35))
- Webpack support for css. ([41f7882](https://github.com/Oriflame/lumos/commit/41f7882))
- **[lumos,jest]** Cross env and script update. ([2d6de27](https://github.com/Oriflame/lumos/commit/2d6de27))
- **[typescript,lumos]** Config. ([4fb2d6e](https://github.com/Oriflame/lumos/commit/4fb2d6e))

#### 📘 Docs

- added lumos docs. ([d37583f](https://github.com/Oriflame/lumos/commit/d37583f))
- Update docu. (#115) ([4faa57c](https://github.com/Oriflame/lumos/commit/4faa57c)), closes [#115](https://github.com/Oriflame/lumos/issues/115)
- Update documentation. ([b811f04](https://github.com/Oriflame/lumos/commit/b811f04))
- Update documentation. ([ffd09f3](https://github.com/Oriflame/lumos/commit/ffd09f3))
- Update readme. ([5f3a707](https://github.com/Oriflame/lumos/commit/5f3a707))
- Updated docs. ([f7ca5e5](https://github.com/Oriflame/lumos/commit/f7ca5e5))

#### 📦 Dependencies

- bump @actions/core from 1.2.5 to 1.2.6 (#695) ([273dd63](https://github.com/Oriflame/lumos/commit/273dd63)), closes [#695](https://github.com/Oriflame/lumos/issues/695)
- bump @beemo/cli from 1.0.10 to 1.0.11 ([42ade32](https://github.com/Oriflame/lumos/commit/42ade32))
- bump @beemo/cli from 1.0.5 to 1.0.6 ([5f92a5c](https://github.com/Oriflame/lumos/commit/5f92a5c))
- bump @beemo/cli from 1.0.6 to 1.0.7 ([3f11b6a](https://github.com/Oriflame/lumos/commit/3f11b6a))
- bump @beemo/cli from 1.0.7 to 1.0.8 ([717e047](https://github.com/Oriflame/lumos/commit/717e047))
- bump @beemo/cli from 1.0.8 to 1.0.9 ([a5d1656](https://github.com/Oriflame/lumos/commit/a5d1656))
- bump @beemo/cli from 1.0.9 to 1.0.10 ([9f9cd6a](https://github.com/Oriflame/lumos/commit/9f9cd6a))
- bump @beemo/core from 1.0.7 to 1.0.8 ([ae428aa](https://github.com/Oriflame/lumos/commit/ae428aa))
- bump @beemo/core from 1.0.8 to 1.0.9 ([059451a](https://github.com/Oriflame/lumos/commit/059451a))
- bump @beemo/core from 1.1.0 to 1.1.1 ([ba080f5](https://github.com/Oriflame/lumos/commit/ba080f5))
- bump @beemo/core from 1.1.1 to 1.1.2 ([027f5cf](https://github.com/Oriflame/lumos/commit/027f5cf))
- bump @beemo/core from 1.1.2 to 1.1.4 ([f35f30d](https://github.com/Oriflame/lumos/commit/f35f30d))
- bump @beemo/core from 1.1.4 to 1.1.5 ([d5fb666](https://github.com/Oriflame/lumos/commit/d5fb666))
- bump @beemo/core from 1.1.5 to 1.1.6 ([900e51a](https://github.com/Oriflame/lumos/commit/900e51a))
- bump @beemo/core from 1.1.6 to 1.1.7 ([610e93a](https://github.com/Oriflame/lumos/commit/610e93a))
- bump @beemo/core from 1.1.7 to 1.1.8 ([202c5da](https://github.com/Oriflame/lumos/commit/202c5da))
- bump @rajzik/conventional-changelog-beemo from 2.0.1 to 2.0.2 ([fda1464](https://github.com/Oriflame/lumos/commit/fda1464))
- bump @rajzik/conventional-changelog-beemo from 2.0.2 to 2.0.4 (#480) ([1ba1499](https://github.com/Oriflame/lumos/commit/1ba1499)), closes [#480](https://github.com/Oriflame/lumos/issues/480)
- bump @rajzik/conventional-changelog-beemo from 2.0.4 to 2.0.5 (#484) ([27504ec](https://github.com/Oriflame/lumos/commit/27504ec)), closes [#484](https://github.com/Oriflame/lumos/issues/484)
- bump @rajzik/conventional-changelog-beemo from 2.0.5 to 2.0.6 (#500) ([2004686](https://github.com/Oriflame/lumos/commit/2004686)), closes [#500](https://github.com/Oriflame/lumos/issues/500)
- bump @rajzik/conventional-changelog-beemo from 2.0.6 to 2.1.0 (#604) ([50cf7a1](https://github.com/Oriflame/lumos/commit/50cf7a1)), closes [#604](https://github.com/Oriflame/lumos/issues/604)
- bump @rajzik/conventional-changelog-beemo from 2.1.0 to 2.1.1 (#679) ([b39094d](https://github.com/Oriflame/lumos/commit/b39094d)), closes [#679](https://github.com/Oriflame/lumos/issues/679)
- bump chalk from 2.4.2 to 3.0.0 ([69a8746](https://github.com/Oriflame/lumos/commit/69a8746))
- bump chalk from 2.4.2 to 3.0.0 ([4d2dc31](https://github.com/Oriflame/lumos/commit/4d2dc31))
- bump chalk from 3.0.0 to 4.0.0 ([a33935c](https://github.com/Oriflame/lumos/commit/a33935c))
- bump chalk from 4.0.0 to 4.1.0 (#402) ([3e56b0d](https://github.com/Oriflame/lumos/commit/3e56b0d)), closes [#402](https://github.com/Oriflame/lumos/issues/402)
- bump check-node-version from 4.0.1 to 4.0.2 ([9857be2](https://github.com/Oriflame/lumos/commit/9857be2))
- bump check-node-version from 4.0.2 to 4.0.3 ([20c5882](https://github.com/Oriflame/lumos/commit/20c5882))
- bump conventional-changelog-beemo from 1.5.1 to 1.5.3 ([765ba19](https://github.com/Oriflame/lumos/commit/765ba19))
- bump conventional-changelog-beemo from 1.5.3 to 1.6.0 ([a699ed1](https://github.com/Oriflame/lumos/commit/a699ed1))
- bump cross-env from 6.0.3 to 7.0.0 ([cb3195b](https://github.com/Oriflame/lumos/commit/cb3195b))
- bump cross-env from 7.0.0 to 7.0.1 ([1da5f20](https://github.com/Oriflame/lumos/commit/1da5f20))
- bump cross-env from 7.0.1 to 7.0.2 ([3ca795c](https://github.com/Oriflame/lumos/commit/3ca795c))
- bump edit-json-file from 1.3.2 to 1.4.0 ([9d6657a](https://github.com/Oriflame/lumos/commit/9d6657a))
- bump edit-json-file from 1.4.0 to 1.4.1 (#566) ([99f53a4](https://github.com/Oriflame/lumos/commit/99f53a4)), closes [#566](https://github.com/Oriflame/lumos/issues/566)
- bump enquirer from 2.3.2 to 2.3.4 ([483f3c6](https://github.com/Oriflame/lumos/commit/483f3c6))
- bump enquirer from 2.3.4 to 2.3.5 ([8a91ace](https://github.com/Oriflame/lumos/commit/8a91ace))
- bump enquirer from 2.3.5 to 2.3.6 (#465) ([77145c2](https://github.com/Oriflame/lumos/commit/77145c2)), closes [#465](https://github.com/Oriflame/lumos/issues/465)
- bump execa from 3.4.0 to 4.0.0 ([7c3dc3f](https://github.com/Oriflame/lumos/commit/7c3dc3f))
- bump execa from 4.0.0 to 4.0.1 ([a4692e9](https://github.com/Oriflame/lumos/commit/a4692e9))
- bump execa from 4.0.1 to 4.0.2 ([411ffac](https://github.com/Oriflame/lumos/commit/411ffac))
- bump execa from 4.0.2 to 4.0.3 (#476) ([53321a0](https://github.com/Oriflame/lumos/commit/53321a0)), closes [#476](https://github.com/Oriflame/lumos/issues/476)

#### 📋 Misc

- **[dotfiles]** added some dotfiles. ([5eb5853](https://github.com/Oriflame/lumos/commit/5eb5853))
- bump @beemo/core from 1.1.0 to 1.1.1 ([48bf1f3](https://github.com/Oriflame/lumos/commit/48bf1f3))
- bump conventional-changelog-beemo from 1.5.1 to 1.5.3 ([6a36a03](https://github.com/Oriflame/lumos/commit/6a36a03))
- Run eslint fix. (#648) ([e645e17](https://github.com/Oriflame/lumos/commit/e645e17)), closes [#648](https://github.com/Oriflame/lumos/issues/648)

#### 🛠 Internals

- Change structure. ([5d4a08a](https://github.com/Oriflame/lumos/commit/5d4a08a))
- changed node version. ([5b02eeb](https://github.com/Oriflame/lumos/commit/5b02eeb))
- cleanup ([9542711](https://github.com/Oriflame/lumos/commit/9542711))
- code style. ([6f798e7](https://github.com/Oriflame/lumos/commit/6f798e7))
- Fix bin. ([ac4f10f](https://github.com/Oriflame/lumos/commit/ac4f10f))
- Fix typo. ([94efc21](https://github.com/Oriflame/lumos/commit/94efc21))
- fix. ([9a30ee7](https://github.com/Oriflame/lumos/commit/9a30ee7))
- fix. ([662f6d5](https://github.com/Oriflame/lumos/commit/662f6d5))
- Fix. ([16ea755](https://github.com/Oriflame/lumos/commit/16ea755))
- fixed access mode. ([037e597](https://github.com/Oriflame/lumos/commit/037e597))
- fixed package errors. ([574845c](https://github.com/Oriflame/lumos/commit/574845c))
- fixed versions. ([c3fd633](https://github.com/Oriflame/lumos/commit/c3fd633))
- merge resolution. ([56c8e7f](https://github.com/Oriflame/lumos/commit/56c8e7f))
- merge. ([c1f6a55](https://github.com/Oriflame/lumos/commit/c1f6a55))
- Migrate to travis-ci. ([cc8649e](https://github.com/Oriflame/lumos/commit/cc8649e))
- Migrated to tests instead of test. ([15c34bd](https://github.com/Oriflame/lumos/commit/15c34bd))
- Move to src folder. ([5a5a48e](https://github.com/Oriflame/lumos/commit/5a5a48e))
- New pipelines. (#642) ([1a593cd](https://github.com/Oriflame/lumos/commit/1a593cd)), closes [#642](https://github.com/Oriflame/lumos/issues/642)
- release [ci skip]. ([b7e2e05](https://github.com/Oriflame/lumos/commit/b7e2e05))
- release [ci skip]. ([1f3eda8](https://github.com/Oriflame/lumos/commit/1f3eda8))
- release [ci skip]. ([f4477dc](https://github.com/Oriflame/lumos/commit/f4477dc))
- release [ci skip]. ([71ab17d](https://github.com/Oriflame/lumos/commit/71ab17d))
- release [ci skip]. ([b98010e](https://github.com/Oriflame/lumos/commit/b98010e))
- release [ci skip]. ([3461627](https://github.com/Oriflame/lumos/commit/3461627))
- release [ci skip]. ([424da5e](https://github.com/Oriflame/lumos/commit/424da5e))
- release [ci skip]. ([5097c2e](https://github.com/Oriflame/lumos/commit/5097c2e))
- release [ci skip]. ([aa50b57](https://github.com/Oriflame/lumos/commit/aa50b57))
- release [ci skip]. ([490ad50](https://github.com/Oriflame/lumos/commit/490ad50))
- release [ci skip]. ([978743b](https://github.com/Oriflame/lumos/commit/978743b))
- release [ci skip]. ([24c5f18](https://github.com/Oriflame/lumos/commit/24c5f18))
- release [ci skip]. ([b18ed68](https://github.com/Oriflame/lumos/commit/b18ed68))
- release [ci skip]. ([d4dad66](https://github.com/Oriflame/lumos/commit/d4dad66))
- release [ci skip]. ([7a2caab](https://github.com/Oriflame/lumos/commit/7a2caab))
- release [ci skip]. ([af929e9](https://github.com/Oriflame/lumos/commit/af929e9))
- release [ci skip]. ([f4e976e](https://github.com/Oriflame/lumos/commit/f4e976e))
- release [ci skip]. ([7d1582c](https://github.com/Oriflame/lumos/commit/7d1582c))
- release [ci skip]. ([38ba08b](https://github.com/Oriflame/lumos/commit/38ba08b))
- release [ci skip]. ([3f3ec2e](https://github.com/Oriflame/lumos/commit/3f3ec2e))
- release [ci skip]. ([32ba9b9](https://github.com/Oriflame/lumos/commit/32ba9b9))
- release [ci skip]. ([d845179](https://github.com/Oriflame/lumos/commit/d845179))
- release [ci skip]. ([baad588](https://github.com/Oriflame/lumos/commit/baad588))
- release [ci skip]. ([d7440a1](https://github.com/Oriflame/lumos/commit/d7440a1))
- release [ci skip]. ([20ece88](https://github.com/Oriflame/lumos/commit/20ece88))
- release [ci skip]. ([8a6b508](https://github.com/Oriflame/lumos/commit/8a6b508))
- release [ci skip]. ([5c2a6a2](https://github.com/Oriflame/lumos/commit/5c2a6a2))
- release [ci skip]. ([f14aac5](https://github.com/Oriflame/lumos/commit/f14aac5))
- release [ci skip]. ([f275bc7](https://github.com/Oriflame/lumos/commit/f275bc7))
- release [ci skip]. ([5d8df6e](https://github.com/Oriflame/lumos/commit/5d8df6e))
- release [ci skip]. ([8950e01](https://github.com/Oriflame/lumos/commit/8950e01))
- release [ci skip]. ([5f80196](https://github.com/Oriflame/lumos/commit/5f80196))
- release [ci skip]. ([5ca4a5a](https://github.com/Oriflame/lumos/commit/5ca4a5a))
- release [ci skip]. ([503ab74](https://github.com/Oriflame/lumos/commit/503ab74))
- release [ci skip]. ([5d7a086](https://github.com/Oriflame/lumos/commit/5d7a086))
- release [ci skip]. ([ee3e47c](https://github.com/Oriflame/lumos/commit/ee3e47c))
- release [ci skip]. ([c514b09](https://github.com/Oriflame/lumos/commit/c514b09))
- release [ci skip]. ([92ce9cd](https://github.com/Oriflame/lumos/commit/92ce9cd))
- release [ci skip]. ([79350fd](https://github.com/Oriflame/lumos/commit/79350fd))
- release [ci skip]. ([6c34286](https://github.com/Oriflame/lumos/commit/6c34286))
- release [ci skip]. ([8ca6a09](https://github.com/Oriflame/lumos/commit/8ca6a09))
- release [ci skip]. ([9caf5fe](https://github.com/Oriflame/lumos/commit/9caf5fe))
- release [ci skip]. ([e51273f](https://github.com/Oriflame/lumos/commit/e51273f))
- release [ci skip]. ([6049229](https://github.com/Oriflame/lumos/commit/6049229))
- release [ci skip]. ([d849cfe](https://github.com/Oriflame/lumos/commit/d849cfe))
- release [ci skip]. ([d13d209](https://github.com/Oriflame/lumos/commit/d13d209))
- release [ci skip]. ([9a62827](https://github.com/Oriflame/lumos/commit/9a62827))
- release [ci skip]. ([7815e94](https://github.com/Oriflame/lumos/commit/7815e94))
- release [ci skip]. ([b3b0b0c](https://github.com/Oriflame/lumos/commit/b3b0b0c))
- release [ci skip]. ([0b3139c](https://github.com/Oriflame/lumos/commit/0b3139c))
- release [ci skip]. ([0fe8a5e](https://github.com/Oriflame/lumos/commit/0fe8a5e))
- release [ci skip]. ([9f8c607](https://github.com/Oriflame/lumos/commit/9f8c607))
- release [ci skip]. ([d853153](https://github.com/Oriflame/lumos/commit/d853153))
- release [ci skip]. ([8fcd95d](https://github.com/Oriflame/lumos/commit/8fcd95d))
- release [ci skip]. ([04996a6](https://github.com/Oriflame/lumos/commit/04996a6))
- release [ci skip]. ([be6f4b9](https://github.com/Oriflame/lumos/commit/be6f4b9))
- release [ci skip]. ([437b42e](https://github.com/Oriflame/lumos/commit/437b42e))
- release [ci skip]. ([2719eb8](https://github.com/Oriflame/lumos/commit/2719eb8))
- release [ci skip]. ([e3eb6f1](https://github.com/Oriflame/lumos/commit/e3eb6f1))
- release [ci skip]. ([fa4336d](https://github.com/Oriflame/lumos/commit/fa4336d))
- release [ci skip]. ([f1f989a](https://github.com/Oriflame/lumos/commit/f1f989a))
- release [ci skip]. ([6ddb93e](https://github.com/Oriflame/lumos/commit/6ddb93e))
- release [ci skip]. ([f704782](https://github.com/Oriflame/lumos/commit/f704782))
- release [ci skip]. ([58d5b8a](https://github.com/Oriflame/lumos/commit/58d5b8a))
- release [ci skip]. ([23e6176](https://github.com/Oriflame/lumos/commit/23e6176))
- release [ci skip]. ([32ca120](https://github.com/Oriflame/lumos/commit/32ca120))
- release [ci skip]. ([9f0cb88](https://github.com/Oriflame/lumos/commit/9f0cb88))
- release [ci skip]. ([e6c6907](https://github.com/Oriflame/lumos/commit/e6c6907))
- release [ci skip]. ([ebc9b86](https://github.com/Oriflame/lumos/commit/ebc9b86))
- release [ci skip]. ([6f661a4](https://github.com/Oriflame/lumos/commit/6f661a4))
- release [ci skip]. ([da41dd8](https://github.com/Oriflame/lumos/commit/da41dd8))
- release [ci skip]. ([2bde0cd](https://github.com/Oriflame/lumos/commit/2bde0cd))
- release [ci skip]. ([09fade2](https://github.com/Oriflame/lumos/commit/09fade2))
- release [ci skip]. ([f011d33](https://github.com/Oriflame/lumos/commit/f011d33))
- release [ci skip]. ([82b5287](https://github.com/Oriflame/lumos/commit/82b5287))
- release [ci skip]. ([83832c3](https://github.com/Oriflame/lumos/commit/83832c3))
- release [ci skip]. ([1db47f8](https://github.com/Oriflame/lumos/commit/1db47f8))
- release [ci skip]. ([5f8e4a3](https://github.com/Oriflame/lumos/commit/5f8e4a3))
- release [ci skip]. ([b2c90b4](https://github.com/Oriflame/lumos/commit/b2c90b4))
- release [ci skip]. ([9ea367f](https://github.com/Oriflame/lumos/commit/9ea367f))
- release [ci skip]. ([da5b8c3](https://github.com/Oriflame/lumos/commit/da5b8c3))
- release [ci skip]. ([37df11d](https://github.com/Oriflame/lumos/commit/37df11d))
- release [ci skip]. ([280b26c](https://github.com/Oriflame/lumos/commit/280b26c))
- release [ci skip]. ([dcde361](https://github.com/Oriflame/lumos/commit/dcde361))
- release [ci skip]. ([d6ebe9b](https://github.com/Oriflame/lumos/commit/d6ebe9b))
- release [ci skip]. ([0dd6a37](https://github.com/Oriflame/lumos/commit/0dd6a37))
- release [ci skip]. ([3711a1b](https://github.com/Oriflame/lumos/commit/3711a1b))
- release [ci skip]. ([9ab94e1](https://github.com/Oriflame/lumos/commit/9ab94e1))
- release [ci skip]. ([b3b0982](https://github.com/Oriflame/lumos/commit/b3b0982))
- release [ci skip]. ([661559a](https://github.com/Oriflame/lumos/commit/661559a))
- release [ci skip]. ([cbc0dbd](https://github.com/Oriflame/lumos/commit/cbc0dbd))
- release [ci skip]. ([509d6e8](https://github.com/Oriflame/lumos/commit/509d6e8))
- release [ci skip]. ([9fc6168](https://github.com/Oriflame/lumos/commit/9fc6168))
- release [ci skip]. ([baba542](https://github.com/Oriflame/lumos/commit/baba542))
- release [ci skip]. ([bf8b092](https://github.com/Oriflame/lumos/commit/bf8b092))
- release [ci skip]. ([e52a584](https://github.com/Oriflame/lumos/commit/e52a584))
- release [ci skip]. ([3e40a0f](https://github.com/Oriflame/lumos/commit/3e40a0f))
- release [ci skip]. ([3c38f6c](https://github.com/Oriflame/lumos/commit/3c38f6c))
- release [ci skip]. ([9069be3](https://github.com/Oriflame/lumos/commit/9069be3))
- release [ci skip]. ([da0f0b9](https://github.com/Oriflame/lumos/commit/da0f0b9))
- release [ci skip]. ([f165264](https://github.com/Oriflame/lumos/commit/f165264))
- release [ci skip]. ([2f1a170](https://github.com/Oriflame/lumos/commit/2f1a170))
- release [ci skip]. ([fa73168](https://github.com/Oriflame/lumos/commit/fa73168))
- release [ci skip]. ([d9c6e69](https://github.com/Oriflame/lumos/commit/d9c6e69))
- release [ci skip]. ([b914a5e](https://github.com/Oriflame/lumos/commit/b914a5e))
- release [ci skip]. ([5513cbf](https://github.com/Oriflame/lumos/commit/5513cbf))
- release [ci skip]. ([1bee887](https://github.com/Oriflame/lumos/commit/1bee887))
- release [ci skip]. ([a6dd211](https://github.com/Oriflame/lumos/commit/a6dd211))
- release [ci skip]. ([6340ece](https://github.com/Oriflame/lumos/commit/6340ece))
- release [ci skip]. ([69a38d5](https://github.com/Oriflame/lumos/commit/69a38d5))
- release [ci skip]. ([bbf9dbf](https://github.com/Oriflame/lumos/commit/bbf9dbf))
- release [ci skip]. ([70dbabf](https://github.com/Oriflame/lumos/commit/70dbabf))
- release [ci skip]. ([cc8b4bc](https://github.com/Oriflame/lumos/commit/cc8b4bc))
- release [ci skip]. ([3b01295](https://github.com/Oriflame/lumos/commit/3b01295))
- release [ci skip]. ([0ff7af1](https://github.com/Oriflame/lumos/commit/0ff7af1))
- release [ci skip]. ([1ab8ab2](https://github.com/Oriflame/lumos/commit/1ab8ab2))
- release [ci skip]. ([8bca37a](https://github.com/Oriflame/lumos/commit/8bca37a))
- release [ci skip]. ([8f8d252](https://github.com/Oriflame/lumos/commit/8f8d252))
- release [ci skip]. ([50dcfb2](https://github.com/Oriflame/lumos/commit/50dcfb2))
- release [ci skip]. ([77956a8](https://github.com/Oriflame/lumos/commit/77956a8))
- release [ci skip]. ([4bf2df4](https://github.com/Oriflame/lumos/commit/4bf2df4))
- release [ci skip]. ([d7351fd](https://github.com/Oriflame/lumos/commit/d7351fd))
- release [ci skip]. ([39bf9d9](https://github.com/Oriflame/lumos/commit/39bf9d9))
- release [ci skip]. ([1f18e8c](https://github.com/Oriflame/lumos/commit/1f18e8c))
- release [ci skip]. ([3c44ff4](https://github.com/Oriflame/lumos/commit/3c44ff4))
- release [ci skip]. ([b311a7c](https://github.com/Oriflame/lumos/commit/b311a7c))
- release [ci skip]. ([233952c](https://github.com/Oriflame/lumos/commit/233952c))
- release [ci skip]. ([7a27908](https://github.com/Oriflame/lumos/commit/7a27908))
- release [ci skip]. ([d7cf240](https://github.com/Oriflame/lumos/commit/d7cf240))
- release [ci skip]. ([66d892c](https://github.com/Oriflame/lumos/commit/66d892c))
- release [ci skip]. ([4f1b9ce](https://github.com/Oriflame/lumos/commit/4f1b9ce))
- release [ci skip]. ([f81e996](https://github.com/Oriflame/lumos/commit/f81e996))
- release [ci skip]. ([1c4baae](https://github.com/Oriflame/lumos/commit/1c4baae))
- release [ci skip]. ([a49a773](https://github.com/Oriflame/lumos/commit/a49a773))
- release [ci skip]. ([3f8e9e3](https://github.com/Oriflame/lumos/commit/3f8e9e3))
- release [ci skip]. ([ef04223](https://github.com/Oriflame/lumos/commit/ef04223))
- release [ci skip]. ([a05ca60](https://github.com/Oriflame/lumos/commit/a05ca60))
- release [ci skip]. ([d2e4ea7](https://github.com/Oriflame/lumos/commit/d2e4ea7))
- release [ci skip]. ([9cf8e90](https://github.com/Oriflame/lumos/commit/9cf8e90))
- release [ci skip]. ([f43eccb](https://github.com/Oriflame/lumos/commit/f43eccb))
- release [ci skip]. ([90e35a9](https://github.com/Oriflame/lumos/commit/90e35a9))
- release [ci skip]. ([139c21a](https://github.com/Oriflame/lumos/commit/139c21a))
- release [ci skip]. ([507a715](https://github.com/Oriflame/lumos/commit/507a715))
- release [ci skip]. ([990a6df](https://github.com/Oriflame/lumos/commit/990a6df))
- release [ci skip]. ([dc3245e](https://github.com/Oriflame/lumos/commit/dc3245e))
- release [ci skip]. ([b2eec0d](https://github.com/Oriflame/lumos/commit/b2eec0d))
- release [ci skip]. ([e4d587b](https://github.com/Oriflame/lumos/commit/e4d587b))
- release [ci skip]. ([4e24ed6](https://github.com/Oriflame/lumos/commit/4e24ed6))
- release [ci skip]. ([63d6d2e](https://github.com/Oriflame/lumos/commit/63d6d2e))
- release [ci skip]. ([ce9c147](https://github.com/Oriflame/lumos/commit/ce9c147))
- release [ci skip]. ([0147219](https://github.com/Oriflame/lumos/commit/0147219))
- release [ci skip]. ([89da4ef](https://github.com/Oriflame/lumos/commit/89da4ef))
- release [ci skip]. ([98be6d2](https://github.com/Oriflame/lumos/commit/98be6d2))
- release [ci skip]. ([3ab089a](https://github.com/Oriflame/lumos/commit/3ab089a))
- release [ci skip]. ([a3adf11](https://github.com/Oriflame/lumos/commit/a3adf11))
- release [ci skip]. ([ccb3b90](https://github.com/Oriflame/lumos/commit/ccb3b90))
- release [ci skip]. ([bc7fc2f](https://github.com/Oriflame/lumos/commit/bc7fc2f))
- release [ci skip]. ([2ce5acb](https://github.com/Oriflame/lumos/commit/2ce5acb))
- release [ci skip]. ([b1966c6](https://github.com/Oriflame/lumos/commit/b1966c6))
- release [ci skip]. ([6720b98](https://github.com/Oriflame/lumos/commit/6720b98))
- release [ci skip]. ([8b9be67](https://github.com/Oriflame/lumos/commit/8b9be67))
- release [ci skip]. ([19c89e2](https://github.com/Oriflame/lumos/commit/19c89e2))
- release [ci skip]. ([6ac4cc6](https://github.com/Oriflame/lumos/commit/6ac4cc6))
- release [ci skip]. ([a12a584](https://github.com/Oriflame/lumos/commit/a12a584))
- release [ci skip]. ([c79b050](https://github.com/Oriflame/lumos/commit/c79b050))
- release [ci skip]. ([12e3c75](https://github.com/Oriflame/lumos/commit/12e3c75))
- release [ci skip]. ([c3c9672](https://github.com/Oriflame/lumos/commit/c3c9672))
- release [ci skip]. ([8f384fa](https://github.com/Oriflame/lumos/commit/8f384fa))
- release [ci skip]. ([9b8f4ef](https://github.com/Oriflame/lumos/commit/9b8f4ef))
- release [ci skip]. ([c19fe23](https://github.com/Oriflame/lumos/commit/c19fe23))
- release [ci skip]. ([d1e60d9](https://github.com/Oriflame/lumos/commit/d1e60d9))
- release [ci skip]. ([eac96c9](https://github.com/Oriflame/lumos/commit/eac96c9))
- release [ci skip]. ([93f4017](https://github.com/Oriflame/lumos/commit/93f4017))
- release [ci skip]. ([f28df15](https://github.com/Oriflame/lumos/commit/f28df15))
- release [ci skip]. ([6a38638](https://github.com/Oriflame/lumos/commit/6a38638))
- release [ci skip]. ([8f66688](https://github.com/Oriflame/lumos/commit/8f66688))
- release [ci skip]. ([8980aa3](https://github.com/Oriflame/lumos/commit/8980aa3))
- release [ci skip]. ([6c42ac3](https://github.com/Oriflame/lumos/commit/6c42ac3))
- release [ci skip]. ([5ae7851](https://github.com/Oriflame/lumos/commit/5ae7851))
- release [ci skip]. ([60b8ac3](https://github.com/Oriflame/lumos/commit/60b8ac3))
- release [ci skip]. ([9c40a51](https://github.com/Oriflame/lumos/commit/9c40a51))
- release [ci skip]. ([4a79d06](https://github.com/Oriflame/lumos/commit/4a79d06))
- release [ci skip]. ([21d149c](https://github.com/Oriflame/lumos/commit/21d149c))
- release [ci skip]. ([6263e29](https://github.com/Oriflame/lumos/commit/6263e29))
- release [ci skip]. ([6ab4083](https://github.com/Oriflame/lumos/commit/6ab4083))
- release [ci skip]. ([074c467](https://github.com/Oriflame/lumos/commit/074c467))
- release [ci skip]. ([e2c272a](https://github.com/Oriflame/lumos/commit/e2c272a))
- release [ci skip]. ([c5ff00e](https://github.com/Oriflame/lumos/commit/c5ff00e))
- release [ci skip]. ([f6addc8](https://github.com/Oriflame/lumos/commit/f6addc8))
- release [ci skip]. ([aacb448](https://github.com/Oriflame/lumos/commit/aacb448))
- release [ci skip]. ([6ddb979](https://github.com/Oriflame/lumos/commit/6ddb979))
- release [ci skip]. ([f428f86](https://github.com/Oriflame/lumos/commit/f428f86))
- release [ci skip]. ([191cc6f](https://github.com/Oriflame/lumos/commit/191cc6f))
- release [ci skip]. ([de90cb7](https://github.com/Oriflame/lumos/commit/de90cb7))
- release [ci skip]. ([4cd454f](https://github.com/Oriflame/lumos/commit/4cd454f))
- release [ci skip]. ([1a822c5](https://github.com/Oriflame/lumos/commit/1a822c5))
- release [ci skip]. ([83df171](https://github.com/Oriflame/lumos/commit/83df171))
- release [ci skip]. ([f83f64d](https://github.com/Oriflame/lumos/commit/f83f64d))
- release [ci skip]. ([f7df801](https://github.com/Oriflame/lumos/commit/f7df801))
- release [ci skip]. ([e26daae](https://github.com/Oriflame/lumos/commit/e26daae))
- release [ci skip]. ([d9644a4](https://github.com/Oriflame/lumos/commit/d9644a4))
- release [ci skip]. ([efcd147](https://github.com/Oriflame/lumos/commit/efcd147))
- release [ci skip]. ([c64843b](https://github.com/Oriflame/lumos/commit/c64843b))
- release [ci skip]. ([f042991](https://github.com/Oriflame/lumos/commit/f042991))
- release [ci skip]. ([918db84](https://github.com/Oriflame/lumos/commit/918db84))
- release [ci skip]. ([5414e4e](https://github.com/Oriflame/lumos/commit/5414e4e))
- release [ci skip]. ([5900111](https://github.com/Oriflame/lumos/commit/5900111))
- release [ci skip]. ([811c2fe](https://github.com/Oriflame/lumos/commit/811c2fe))
- release [ci skip]. ([8dc25b6](https://github.com/Oriflame/lumos/commit/8dc25b6))
- release [ci skip]. ([92a9c15](https://github.com/Oriflame/lumos/commit/92a9c15))
- release [ci skip]. ([514d53d](https://github.com/Oriflame/lumos/commit/514d53d))
- release [ci skip]. ([37def70](https://github.com/Oriflame/lumos/commit/37def70))
- release [ci skip]. ([b0adc97](https://github.com/Oriflame/lumos/commit/b0adc97))
- release [ci skip]. ([8f0bb7c](https://github.com/Oriflame/lumos/commit/8f0bb7c))
- release [ci skip]. ([9a0c106](https://github.com/Oriflame/lumos/commit/9a0c106))
- release [ci skip]. ([44a4a65](https://github.com/Oriflame/lumos/commit/44a4a65))
- release [ci skip]. ([26ce7c4](https://github.com/Oriflame/lumos/commit/26ce7c4))
- release [ci skip]. ([a98154a](https://github.com/Oriflame/lumos/commit/a98154a))
- release [ci skip]. ([34115a5](https://github.com/Oriflame/lumos/commit/34115a5))
- release [ci skip]. ([f066b97](https://github.com/Oriflame/lumos/commit/f066b97))
- release [ci skip]. ([f0d49ad](https://github.com/Oriflame/lumos/commit/f0d49ad))
- release [ci skip]. ([d49ee48](https://github.com/Oriflame/lumos/commit/d49ee48))
- release [ci skip]. ([53966de](https://github.com/Oriflame/lumos/commit/53966de))
- release [ci skip]. ([0122f5c](https://github.com/Oriflame/lumos/commit/0122f5c))
- release [ci skip]. ([c3062ec](https://github.com/Oriflame/lumos/commit/c3062ec))
- release [ci skip]. ([fa57859](https://github.com/Oriflame/lumos/commit/fa57859))
- release [ci skip]. ([7d8c5da](https://github.com/Oriflame/lumos/commit/7d8c5da))
- release [ci skip]. ([48f7598](https://github.com/Oriflame/lumos/commit/48f7598))
- release [ci skip]. ([53ad320](https://github.com/Oriflame/lumos/commit/53ad320))
- release [ci skip]. ([3516fd2](https://github.com/Oriflame/lumos/commit/3516fd2))
- release [ci skip]. ([f3d2b0c](https://github.com/Oriflame/lumos/commit/f3d2b0c))
- release [ci skip]. ([ea73e9a](https://github.com/Oriflame/lumos/commit/ea73e9a))
- release [ci skip]. ([1de0b58](https://github.com/Oriflame/lumos/commit/1de0b58))
- release [ci skip]. ([d476180](https://github.com/Oriflame/lumos/commit/d476180))
- release [ci skip]. ([3a9d081](https://github.com/Oriflame/lumos/commit/3a9d081))
- release [ci skip]. ([92a4518](https://github.com/Oriflame/lumos/commit/92a4518))
- release [ci skip]. ([373113e](https://github.com/Oriflame/lumos/commit/373113e))
- release [ci skip]. ([e85ad52](https://github.com/Oriflame/lumos/commit/e85ad52))
- release [ci skip]. ([4cafdb4](https://github.com/Oriflame/lumos/commit/4cafdb4))
- release [ci skip]. ([3eff835](https://github.com/Oriflame/lumos/commit/3eff835))
- release [ci skip]. ([ad700a1](https://github.com/Oriflame/lumos/commit/ad700a1))
- release [ci skip]. ([abc8e30](https://github.com/Oriflame/lumos/commit/abc8e30))
- release [ci skip]. ([7b2bf6a](https://github.com/Oriflame/lumos/commit/7b2bf6a))
- release [ci skip]. ([21c84e3](https://github.com/Oriflame/lumos/commit/21c84e3))
- release [ci skip]. ([71a06ed](https://github.com/Oriflame/lumos/commit/71a06ed))
- release [ci skip]. ([6b91c0d](https://github.com/Oriflame/lumos/commit/6b91c0d))
- release [ci skip]. ([41da99a](https://github.com/Oriflame/lumos/commit/41da99a))
- release [ci skip]. ([32184e0](https://github.com/Oriflame/lumos/commit/32184e0))
- release [ci skip]. ([d7fd0a0](https://github.com/Oriflame/lumos/commit/d7fd0a0))
- release [ci skip]. ([9563df1](https://github.com/Oriflame/lumos/commit/9563df1))
- release [ci skip]. ([e0a9904](https://github.com/Oriflame/lumos/commit/e0a9904))
- release [ci skip]. ([c986712](https://github.com/Oriflame/lumos/commit/c986712))
- release [ci skip]. ([012a610](https://github.com/Oriflame/lumos/commit/012a610))
- release [ci skip]. ([52dce1a](https://github.com/Oriflame/lumos/commit/52dce1a))
- release [ci skip]. ([41304bd](https://github.com/Oriflame/lumos/commit/41304bd))
- release [ci skip]. ([5023ca6](https://github.com/Oriflame/lumos/commit/5023ca6))
- release [ci skip]. ([8fe1422](https://github.com/Oriflame/lumos/commit/8fe1422))
- release [ci skip]. ([8d833d2](https://github.com/Oriflame/lumos/commit/8d833d2))
- release [ci skip]. ([04124e3](https://github.com/Oriflame/lumos/commit/04124e3))
- release [ci skip]. ([55d0b87](https://github.com/Oriflame/lumos/commit/55d0b87))
- release [ci skip]. ([63a3a48](https://github.com/Oriflame/lumos/commit/63a3a48))
- release [ci skip]. ([29e9d58](https://github.com/Oriflame/lumos/commit/29e9d58))
- release [ci skip]. ([0aaa140](https://github.com/Oriflame/lumos/commit/0aaa140))
- release [ci skip]. ([424a6dd](https://github.com/Oriflame/lumos/commit/424a6dd))
- release [ci skip]. ([bb974e3](https://github.com/Oriflame/lumos/commit/bb974e3))
- release [ci skip]. ([c741186](https://github.com/Oriflame/lumos/commit/c741186))
- release [ci skip]. ([666ef04](https://github.com/Oriflame/lumos/commit/666ef04))
- release [ci skip]. ([fa2b200](https://github.com/Oriflame/lumos/commit/fa2b200))
- release [ci skip]. ([0f6cf70](https://github.com/Oriflame/lumos/commit/0f6cf70))
- release [ci skip]. ([02b37d3](https://github.com/Oriflame/lumos/commit/02b37d3))
- release [ci skip]. ([8adbba9](https://github.com/Oriflame/lumos/commit/8adbba9))
- release [ci skip]. ([d793a26](https://github.com/Oriflame/lumos/commit/d793a26))
- release [ci skip]. ([24f9d53](https://github.com/Oriflame/lumos/commit/24f9d53))
- release [ci skip]. ([9975ca5](https://github.com/Oriflame/lumos/commit/9975ca5))
- release [ci skip]. ([abe30a1](https://github.com/Oriflame/lumos/commit/abe30a1))
- release [ci skip]. ([d7d0dd7](https://github.com/Oriflame/lumos/commit/d7d0dd7))
- release [ci skip]. ([477919d](https://github.com/Oriflame/lumos/commit/477919d))
- release [ci skip]. ([be139b7](https://github.com/Oriflame/lumos/commit/be139b7))
- release [ci skip]. ([155359f](https://github.com/Oriflame/lumos/commit/155359f))
- release [ci skip]. ([e82b0ef](https://github.com/Oriflame/lumos/commit/e82b0ef))
- release [ci skip]. ([a61e3e9](https://github.com/Oriflame/lumos/commit/a61e3e9))
- release [ci skip]. ([2bfc9c0](https://github.com/Oriflame/lumos/commit/2bfc9c0))
- release [ci skip]. ([65c5e22](https://github.com/Oriflame/lumos/commit/65c5e22))
- release [ci skip]. ([467c631](https://github.com/Oriflame/lumos/commit/467c631))
- release [ci skip]. ([7bd6f2d](https://github.com/Oriflame/lumos/commit/7bd6f2d))
- release [ci skip]. ([14a2cb0](https://github.com/Oriflame/lumos/commit/14a2cb0))
- release [ci skip]. ([a237a6c](https://github.com/Oriflame/lumos/commit/a237a6c))
- release [ci skip]. ([a071e11](https://github.com/Oriflame/lumos/commit/a071e11))
- release [ci skip]. ([e879700](https://github.com/Oriflame/lumos/commit/e879700))
- release [ci skip]. ([e28bb3b](https://github.com/Oriflame/lumos/commit/e28bb3b))
- release [ci skip]. ([6688465](https://github.com/Oriflame/lumos/commit/6688465))
- release [ci skip]. ([9cd9dcd](https://github.com/Oriflame/lumos/commit/9cd9dcd))
- release [ci skip]. ([4d4f93b](https://github.com/Oriflame/lumos/commit/4d4f93b))
- release [ci skip]. ([6868f22](https://github.com/Oriflame/lumos/commit/6868f22))
- release [ci skip]. ([902c069](https://github.com/Oriflame/lumos/commit/902c069))
- release [ci skip]. ([98f9b15](https://github.com/Oriflame/lumos/commit/98f9b15))
- release [ci skip]. ([f596ff3](https://github.com/Oriflame/lumos/commit/f596ff3))
- release [ci skip]. ([4e980cd](https://github.com/Oriflame/lumos/commit/4e980cd))
- release [ci skip]. ([a0f9d45](https://github.com/Oriflame/lumos/commit/a0f9d45))
- release [ci skip]. ([d7c27a8](https://github.com/Oriflame/lumos/commit/d7c27a8))
- release [ci skip]. ([06fc1e8](https://github.com/Oriflame/lumos/commit/06fc1e8))
- release [ci skip]. ([22575fc](https://github.com/Oriflame/lumos/commit/22575fc))
- release [ci skip]. ([12311c6](https://github.com/Oriflame/lumos/commit/12311c6))
- release [ci skip]. ([b6a922c](https://github.com/Oriflame/lumos/commit/b6a922c))
- release [ci skip]. ([5e05f97](https://github.com/Oriflame/lumos/commit/5e05f97))
- release [ci skip]. ([3211d9c](https://github.com/Oriflame/lumos/commit/3211d9c))
- release [ci skip]. ([7c93f14](https://github.com/Oriflame/lumos/commit/7c93f14))
- release [ci skip]. ([c60be76](https://github.com/Oriflame/lumos/commit/c60be76))
- release [ci skip]. ([4b50a7b](https://github.com/Oriflame/lumos/commit/4b50a7b))
- release [ci skip]. ([a1c87f8](https://github.com/Oriflame/lumos/commit/a1c87f8))
- release [ci skip]. ([e505377](https://github.com/Oriflame/lumos/commit/e505377))
- release [ci skip]. ([33673eb](https://github.com/Oriflame/lumos/commit/33673eb))
- release [ci skip]. ([1cfdabc](https://github.com/Oriflame/lumos/commit/1cfdabc))
- release [ci skip]. ([b1e3ec2](https://github.com/Oriflame/lumos/commit/b1e3ec2))
- release [ci skip]. ([cb4ee2b](https://github.com/Oriflame/lumos/commit/cb4ee2b))
- release [ci skip]. ([1973ef0](https://github.com/Oriflame/lumos/commit/1973ef0))
- release [ci skip]. ([e04cb2c](https://github.com/Oriflame/lumos/commit/e04cb2c))
- release [ci skip]. ([15719aa](https://github.com/Oriflame/lumos/commit/15719aa))
- release [ci skip]. ([9ea0cf3](https://github.com/Oriflame/lumos/commit/9ea0cf3))
- release [ci skip]. ([b4724ad](https://github.com/Oriflame/lumos/commit/b4724ad))
- release [ci skip]. ([93e4bcb](https://github.com/Oriflame/lumos/commit/93e4bcb))
- release [ci skip]. ([e487f97](https://github.com/Oriflame/lumos/commit/e487f97))
- release [ci skip]. ([33ccc45](https://github.com/Oriflame/lumos/commit/33ccc45))
- release [ci skip]. ([d64ff8d](https://github.com/Oriflame/lumos/commit/d64ff8d))
- release [ci skip]. ([3d72b90](https://github.com/Oriflame/lumos/commit/3d72b90))
- release [ci skip]. ([a77a7dd](https://github.com/Oriflame/lumos/commit/a77a7dd))
- release [ci skip]. ([62886c5](https://github.com/Oriflame/lumos/commit/62886c5))
- release [ci skip]. ([40d1740](https://github.com/Oriflame/lumos/commit/40d1740))
- release [ci skip]. ([79e9b1c](https://github.com/Oriflame/lumos/commit/79e9b1c))
- release [ci skip]. ([b780baf](https://github.com/Oriflame/lumos/commit/b780baf))
- release [ci skip]. ([29b9c24](https://github.com/Oriflame/lumos/commit/29b9c24))
- release [ci skip]. ([aa70f61](https://github.com/Oriflame/lumos/commit/aa70f61))
- release [ci skip]. ([2962adf](https://github.com/Oriflame/lumos/commit/2962adf))
- release [ci skip]. ([2b064b2](https://github.com/Oriflame/lumos/commit/2b064b2))
- release [ci skip]. ([e36bb68](https://github.com/Oriflame/lumos/commit/e36bb68))
- release [ci skip]. ([981bab5](https://github.com/Oriflame/lumos/commit/981bab5))
- release [ci skip]. ([52525e0](https://github.com/Oriflame/lumos/commit/52525e0))
- release [ci skip]. ([90666f1](https://github.com/Oriflame/lumos/commit/90666f1))
- release [ci skip]. ([6aa97b5](https://github.com/Oriflame/lumos/commit/6aa97b5))
- release [ci skip]. ([c1c85a6](https://github.com/Oriflame/lumos/commit/c1c85a6))
- release [ci skip]. ([49c9a79](https://github.com/Oriflame/lumos/commit/49c9a79))
- release [ci skip]. ([f55d04c](https://github.com/Oriflame/lumos/commit/f55d04c))
- release [ci skip]. ([96d278b](https://github.com/Oriflame/lumos/commit/96d278b))
- release [ci skip]. ([aceab1f](https://github.com/Oriflame/lumos/commit/aceab1f))
- release [ci skip]. ([5658082](https://github.com/Oriflame/lumos/commit/5658082))
- release [ci skip]. ([2d3b054](https://github.com/Oriflame/lumos/commit/2d3b054))
- release [ci skip]. ([8a3e47d](https://github.com/Oriflame/lumos/commit/8a3e47d))
- release [ci skip]. ([99990f1](https://github.com/Oriflame/lumos/commit/99990f1))
- release [ci skip]. ([74fc49f](https://github.com/Oriflame/lumos/commit/74fc49f))
- removed commit. ([b8acb58](https://github.com/Oriflame/lumos/commit/b8acb58))
- Update config. ([2b2168a](https://github.com/Oriflame/lumos/commit/2b2168a))
- Update tests. ([f49421a](https://github.com/Oriflame/lumos/commit/f49421a))
- Update tsconfig. ([fb5be86](https://github.com/Oriflame/lumos/commit/fb5be86))
- Update typescript files. ([3433e7f](https://github.com/Oriflame/lumos/commit/3433e7f))
- updated release. ([f18c3c7](https://github.com/Oriflame/lumos/commit/f18c3c7))
- whole packages reset. ([a2b69f8](https://github.com/Oriflame/lumos/commit/a2b69f8))
- Wip. ([23bb0a4](https://github.com/Oriflame/lumos/commit/23bb0a4))
- **[Lumos]** Update commitlint template. ([80cf118](https://github.com/Oriflame/lumos/commit/80cf118))

**Note:** Version bump only for package @oriflame/lumos
